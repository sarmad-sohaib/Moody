...
import com.sarmad.moody.data.local.datasource.weather.FakeWeatherLocalDataSource
import com.sarmad.moody.data.local.entity.weather.WeatherEntity
import com.sarmad.moody.data.network.datasource.weather.FakeWeatherNetworkDataSource
...
class DefaultWeatherRepositoryTest {
    ...
    fun setup() {
        ...
        fakeTimeProvider = FakeTimeProvider(isFixedTime = true)
    }

    @Test
    fun `emits network result and saves locally when local data is missing`() = runTest {
        networkDataSource =
            FakeWeatherNetworkDataSource(scenario = FakeWeatherNetworkDataSource.Scenario.SUCCESS)
    ...
    fun `emits network error when fetching update fails with expired data`() = runTest {
        ...
        localDataSource.setWeather(weather = expiredWeather)
        networkDataSource =
            FakeWeatherNetworkDataSource(scenario = FakeWeatherNetworkDataSource.Scenario.NOT_FOUND)
        repository = DefaultWeatherRepository(
            weatherNetworkDataSource = networkDataSource,
            weatherLocalDataSource = localDataSource,
            timeProvider = fakeTimeProvider,
    ...
    fun `emits cached data when local data is fresh`() = runTest {
        ...
        networkDataSource =
            FakeWeatherNetworkDataSource(scenario = FakeWeatherNetworkDataSource.Scenario.SUCCESS) // Should NOT be called
        repository = DefaultWeatherRepository(
            weatherNetworkDataSource = networkDataSource,
            weatherLocalDataSource = localDataSource,
            timeProvider = fakeTimeProvider,
